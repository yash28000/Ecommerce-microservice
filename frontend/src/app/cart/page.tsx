        ...
        <div>
            <div className="w-full flex justify-between  px-10 mt-5 gap-3">
                ...
                <div className="h-full py-4 gap-5 w-1/4 shadow-md px-2 rounded-md flex flex-col">
                    ...
                    <p>Subtotal for ({data.cart_data.length} items) : Rs{data.total}</p>
                    <Button className="w-full" variant="default">Proceed to Buy</Button>
                </div>
            </div>
            {/**/}
        </div>
...
const CartDetails = ({params,change }:{params:CartData,change: ()=>void}) =>{
    ...
    const handleChange = async (e: string) => {
        ...
        const data_temp = await fetch(`${process.env.NEXT_PUBLIC_API}cart/cart?id=${params.id}`, {
            method: 'PUT',
    ...
    const handleDelete = async() =>{
        const data = await fetch(`${process.env.NEXT_PUBLIC_API}cart/cart?id=${params.id}`, {
            method: 'DELETE'
        }).then((res) => {
            return res.json()   
        }).then((data) => {
    ...
    return data ? (
        <div className="flex items-center w-full gap-4 py-3 px-5 ">
            <img src={`${process.env.NEXT_PUBLIC_API}image/img?id=${data[0]?.id}&name=${data[0]?.name}&type=product`} className="w-40 h-40 object-cover" />
            <div className="flex items-center w-full justify-between">
                <span className="flex flex-col items-start">
                    <p className='text-left font-semibold text-xl my-1 '>{wordlimiter(data[0]?.name,150)}</p>
                    <span className="flex items-center justify-between w-full cursor-pointer my-4">
                    <p className="text-sm font-extralight italic flex items-center gap-2 ">qty: <SelectForm cartDetails={params} handleChange={handleChange}/></p>
